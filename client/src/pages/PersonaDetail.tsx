...
import { ArrowLeft, Brain, FileText, MessageCircle, Sparkles, Upload, Zap } from "lucide-react";
...
export default function PersonaDetailPage() {
  ...
  const ingestMutation = trpc.persona.ingestKnowledge.useMutation({
    onSuccess: (data) => {
      toast.success(`${pd.importSuccess} (${data.chunks} chunks)`);
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <Link href="/app/personas">
        <Button variant="ghost" size="sm" className="gap-2 text-muted-foreground hover:text-foreground mb-6 -ml-2">
          <ArrowLeft className="w-4 h-4" /> {pd.backBtn}
        </Button>
      </Link>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-3">
                {(persona.traits as string[] ?? []).map((trait) => (
                  <span key={trait} className="text-[10px] px-2 py-0.5 rounded-full bg-[oklch(0.65_0.22_285/0.1)] border border-[oklch(0.65_0.22_285/0.2)] text-[oklch(0.75_0.15_285)]">
                    {trait}
                  </span>
                ))}
              </div>
              <div className="flex gap-2 mt-3">
                {persona.autoCommentEnabled && (
                  <Badge variant="outline" className="text-[9px] px-2 border-[oklch(0.7_0.18_155/0.4)] text-[oklch(0.7_0.18_155)] bg-[oklch(0.7_0.18_155/0.08)]">
                    <Zap className="w-2.5 h-2.5 mr-1" /> {pd.autoCommentBadge}
                  </Badge>
                )}
                <Badge variant="outline" className="text-[9px] px-2 border-[oklch(0.65_0.22_285/0.3)] text-[oklch(0.75_0.15_285)]">
                  <Sparkles className="w-2.5 h-2.5 mr-1" /> {pd.securityBadge}
                </Badge>
              </div>
            </div>
            <AlignmentRing score={alignData?.score ?? persona.alignmentScore ?? 0} label={pd.statsAlignment} />
          </div>
        </div>
        <div className="flex flex-col gap-2 shrink-0">
          <Link href={`/app/chat?personaId=${persona.id}`}>
            <Button size="sm" className="gap-2 bg-gradient-to-r from-[oklch(0.65_0.22_285)] to-[oklch(0.55_0.2_295)] hover:opacity-90 text-white border-0 w-full">
              <MessageCircle className="w-4 h-4" /> {pd.chatBtn}
            </Button>
          </Link>
          <div className="text-center text-xs text-muted-foreground">
            {alignData?.memoryCount ?? persona.memoryCount ?? 0} {pd.statsMemory}
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="memories">
        <TabsList className="bg-secondary border border-border mb-6">
          <TabsTrigger value="memories" className="gap-2 data-[state=active]:bg-[oklch(0.65_0.22_285/0.2)] data-[state=active]:text-[oklch(0.8_0.15_285)]">
            <Brain className="w-4 h-4" /> {pd.tabMemory} ({memories?.length ?? 0})
          </TabsTrigger>
          <TabsTrigger value="knowledge" className="gap-2 data-[state=active]:bg-[oklch(0.65_0.22_285/0.2)] data-[state=active]:text-[oklch(0.8_0.15_285)]">
            <FileText className="w-4 h-4" /> {pd.tabKnowledge} ({docs?.length ?? 0})
          </TabsTrigger>
          {isOwner && (
            <TabsTrigger value="ingest" className="gap-2 data-[state=active]:bg-[oklch(0.65_0.22_285/0.2)] data-[state=active]:text-[oklch(0.8_0.15_285)]">
              <Upload className="w-4 h-4" /> {pd.tabImport}
            </TabsTrigger>
          )}
          <TabsTrigger value="search" className="gap-2 data-[state=active]:bg-[oklch(0.65_0.22_285/0.2)] data-[state=active]:text-[oklch(0.8_0.15_285)]">
            <Sparkles className="w-4 h-4" /> {locale === "zh" ? "语义搜索" : "Semantic Search"}
          </TabsTrigger>
        </TabsList>
              ...
              <div key={m.id} className="p-4 rounded-xl border border-border bg-card">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    {locale === "zh" ? "重要度" : "Importance"} {m.importance?.toFixed(1)}
                  </span>
              ...
              <div key={doc.id} className="p-4 rounded-xl border border-border bg-card flex items-start justify-between gap-4">
                ...
                <div className="shrink-0 text-right">
                  <Badge variant="outline" className={`text-[9px] px-1.5 ${doc.status === "done" ? "border-[oklch(0.7_0.18_155/0.4)] text-[oklch(0.7_0.18_155)]" : "border-border text-muted-foreground"}`}>
                    {doc.status === "done" ? `✓ ${doc.chunkCount} chunks` : doc.status}
                  </Badge>
          ...
          <TabsContent value="ingest">
            <div className="p-6 rounded-2xl border border-border bg-card space-y-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-semibold">{pd.importTitle}</h3>
              </div>
              <p className="text-sm text-muted-foreground">
                {"Paste articles, documents, or research content. The system will auto-chunk, vectorize, and store it as persona memory to improve alignment score."}
              </p>
              <div className="space-y-1.5">
                <Label className="text-sm">{pd.importDocTitle}</Label>
                <Input placeholder={pd.importDocTitlePlaceholder} value={knowledgeTitle}
                  onChange={(e) => setKnowledgeTitle(e.target.value)} className="bg-input border-border" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-sm">{pd.importContent}</Label>
                <Textarea placeholder={pd.importContentPlaceholder} value={knowledgeContent}
                  onChange={(e) => setKnowledgeContent(e.target.value)}
                  rows={10} className="bg-input border-border resize-none font-mono text-xs" />
                <div className="text-xs text-muted-foreground text-right">
                  {knowledgeContent.length} chars
                </div>
              </div>
              <Button
                ...
                onClick={() => {
                  if (!knowledgeTitle.trim() || !knowledgeContent.trim()) return toast.error(t.common.required);
        ...
        <TabsContent value="search">
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-sm">{pd.memorySearchPlaceholder}</Label>
              <Input placeholder={pd.memorySearchPlaceholder} value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)} className="bg-input border-border" />
            </div>
            {searchResults && searchResults.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground text-sm">{pd.memorySearchEmpty}</div>
            ) : null}
          </div>
        </TabsContent>
      </Tabs>
